...
export default function handleCommands({ queryClient, command, allQueries }: Props) {
  const currentDataPath = [] as any; // NOT USED FOR DATA EXPLORER
  const query = allQueries.find((query) => query.queryKey[0] === command.queryKey);
  if (!query) {
  ...
  switch (command.command) {
    ...
    case 'Data Update':
      ...
      // Set the new data
      const newData = updateNestedDataByPath(oldData, currentDataPath, serverNewData);
